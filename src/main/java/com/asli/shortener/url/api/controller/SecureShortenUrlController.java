...
@RestController
@RequestMapping("/secure/shortenApi/v1")
public class SecureShortenUrlController {

    private final UrlService urlService;

    public SecureShortenUrlController(UrlService urlService) {
    ...
    public ResponseEntity<BaseResponse> shortenURL(@Valid @RequestBody UrlRequest urlRequestData, HttpServletRequest servletRequest) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/v1/" + key)).body(new BaseResponse(key, BaseResponse.STATUS.SUCCESSFUL));
    }
